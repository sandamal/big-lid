...
def poison_data():
    for poison_per in poison_percentages:
        ...
        batch = np.array(np.concatenate((train_x, train_y), axis=1))

        # lid of a batch of query points X
        def mle_batch_euclidean(data, k):
            """
            ...
            distances = cdist(data, batch)
            # get the closest k neighbours
            sorted_distances = np.apply_along_axis(np.sort, axis=1, arr=distances)[:, 0:k]
            a = np.apply_along_axis(f, axis=1, arr=sorted_distances)
            return np.nan_to_num(a)

        # used for cost calculation
        def fast_lid_cost_calculation(z_bar_update, k=10):
            lid = mle_batch_euclidean(z_bar_update, k=k)
            return lid_cost_coefficient * np.sum(lid)

        # used for gradient calculation
        def lid_cost_batch(z_bar_update, closest_neighbours, k):
            lid_cost = 0
            for i in range(z_bar_update.shape[0]):
                r_max = np.sqrt(np.sum((closest_neighbours[i, -1, :] - z_bar_update[i, :]) ** 2, axis=0))
                sum = 0
                for j in range(closest_neighbours.shape[1]):
                    r_i = np.sqrt(np.sum((closest_neighbours[i, j, :] - z_bar_update[i, :]) ** 2, axis=0))
                    sum += np.log((r_i / r_max) + 1e-9)
                lid = -k / sum
                lid_cost += lid
            lid_cost = lid_cost_coefficient * lid_cost
            return lid_cost
        ...
        def validation_error(z_bar_update):
            ...
            lid_cost = fast_lid_cost_calculation(z_bar_update, lid_k)
            return -mse + lid_cost
        ...
        def calculate_derivative(z_bar_update):
            ...
            a = cdist(z_bar_update, batch)
            sort_indices = np.apply_along_axis(np.argsort, axis=1, arr=a)[:, 0:lid_k]
            neighbors = batch[sort_indices, :].squeeze()
            grad_lid_cost = grad(lid_cost_batch, 0)
            lid_gradient = grad_lid_cost(z_bar_update, neighbors, lid_k)
            gradient = -grad_e_z + lid_gradient
        ...
        for tol in tol_values:
            ...
            directory = f'datasets/{dataset}/adaptive/CV_{CV}/{poison_per}/'
            os.makedirs(directory, exist_ok=True)
            np.savez(f'datasets/{dataset}/adaptive/CV_{CV}/{poison_per}/{file_name}', z_bar_initial=z_bar_initial,
                     z_bar=optimized_z,
                     train_x=train_x,
